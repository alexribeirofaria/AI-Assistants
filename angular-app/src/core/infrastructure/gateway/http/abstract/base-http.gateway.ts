...
import { HttpResponseInterpretation } from "../../../errors/domain/http-response-interpretation";
import { HttpStatusInterpreter } from "../../../errors/mappers/http-status-interpreter";
import { ServiceErrorHandlerService } from "../../../errors/services/service-error-handler.service";
import { IFallbackChainContext, IHttpDependencies } from "../interfaces";
...
export abstract class BaseHttpGateway {
  ...
  constructor(dependencies: IHttpDependencies = {}) {
  ...
  protected async executeWithFallbackChain<THandler, TResult>(
    context: IFallbackChainContext<THandler, TResult>,
  ): Promise<TResult> {
      ...
      try {
        ...
        if (context.validate && !context.validate(result)) {
          lastError = this.buildServiceError(
            "executeWithFallbackChain",
            context.invalidResultMessage ??
              "Resultado inválido na cadeia de fallback",
          );
          continue;
        }
    ...
    throw (
      lastError ??
      this.buildServiceError(
        "executeWithFallbackChain",
        "Falha ao executar cadeia de fallback",
      )
    );
  }

  protected buildServiceError(
    operation: string,
    message: string,
  ...
  protected interpretStatus(statusCode: number): HttpResponseInterpretation {
    return HttpStatusInterpreter.interpret(statusCode);
  }
}

...
import { ChatGatewayChainFactory } from './chat-gateway-chain.factory';
...
describe('ChatGatewayChainHandler Unit Tests', () => {
  ...
  it('delegates to next handler when current handler throws', async () => {
    ...
    const rootHandler = ChatGatewayChainFactory.create([primaryGateway, secondaryGateway]);
  ...
  it('throws when chain factory receives no gateways', () => {
    expect(() => ChatGatewayChainFactory.create([])).toThrowError(
